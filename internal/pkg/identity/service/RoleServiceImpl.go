...
func (r RoleServiceImpl) GetUsers(roleTag string) ([]*entities.User, error) {
	role, err := r.RoleRepository.GetByTag(roleTag)
	if err != nil {
		return nil, errors.New("ce role n'existe pas")
	}
	return r.UserRepository.GetUsersByRoleID(role.ID)
}
